...
sudo ip netns exec router tc filter add dev ifbcs parent 1: protocol ip prio 1 u32 match ip dst $SERVER1_IP action skbedit queue_mapping 0
sudo ip netns exec router tc filter add dev ifbcs parent 1: protocol ip prio 2 matchall action skbedit queue_mapping 1

sudo ip netns exec router tc filter add dev ifbsc parent 1: protocol ip prio 1 u32 match ip dst $SERVER1_IP action skbedit queue_mapping 0
